...
The simulation's main objective is to calculate and model the scintillation index latexmath:[\sigma_I^2]. This is the normalized variance of the irradiance. The MATLAB script first determines the point scintillation index, denoted as latexmath:[(\sigma_I^2)_p], for a single-transmitter path, which serves as the primary basis for this simulation. The value latexmath:[(σ_{I,point}^2)] will be calculated using the Rytov variance and is generally considered valid across weak to strong turbulence conditions. 
